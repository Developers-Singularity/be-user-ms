...
async def create_user(session: Session, schema: UserCreate):
...
async def change_password(session: Session, schema: UserChangePassword):
    if found := session.query(User).get(schema.id):
        if SecurityManager.compare_hash(found.password, schema.old_password):
            ...
            raise CustomException(422, "Invalid password", "Old password does not match with the current one.")
    raise CustomException(200, "Resource not found", f"User with ID {schema.id} not found")
